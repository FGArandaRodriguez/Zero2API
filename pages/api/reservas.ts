...
export default async function Reservas(
    ...
    if (req.method === 'GET') {
        ...
        if (!id_cliente || !id_mesa || !fecha_hora || !numero_personas_reserva || !confirmacion) {
            return res.status(400).json({ message: 'Faltan campos requeridos', numero_personas_reserva });
        }
        ...
        try {
            ...
            res.status(500).json({ message: 'Error al crear la reserva o actualizar la mesa', error });
        }
        ...
        if (fecha_hora) {
            ...
            try {
                ...
                return res.status(500).json({ message: 'Error al validar disponibilidad', error });
            }
        }
